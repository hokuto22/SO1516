...
void wvUsername(char (*pedido)[128])
{
...
void shell(){
	int ct = 1024, exit = 0, erro = 0, n = 0, i = 0;
	...
	while(exit == 0){
		...
		if (strcmp(line, "?") == 0){
			erro = write(1, comandos, sizeof(comandos));
		}
...
int main(void)
{
	...
	fifo = open("/home/$USER/.Backup/FIFOs/fifo", O_WRONLY); /* Abrir fifo para enviar mensagens ao servidor */
	if (fifo < 0)
	{
	...
	while (opcao != 9) /* Enquanto o utilizador não escolher exit*/
	{
	    ...
	    switch (opcao)
	    {
		    case 1: /* login */
				...
				if (resposta == 0) /* Se receber 1 número é porque username e 
				    pass estão corretos e a resposta é o número de créditos ainda disponiveis */
				{
